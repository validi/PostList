...
import com.allattentionhere.autoplayvideos.AAH_DownloadService;
...
public class AAH_CustomRecyclerView1 extends RecyclerView {
    ...
    public AAH_CustomRecyclerView1(Context context) {
        super(context);
    }

    public AAH_CustomRecyclerView1(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);


    }

    public AAH_CustomRecyclerView1(Context context, @Nullable AttributeSet attrs, int defStyle) {
...
//        Log.d("trace", "playAvailableVideos: ");
        HandlerThread handlerThread = new HandlerThread("DONT_GIVE_UP", Process.THREAD_PRIORITY_BACKGROUND + Process.THREAD_PRIORITY_MORE_FAVORABLE);
        handlerThread.start();
...
//                        Log.d("pos", "recyclerview left: " + rect_parent.left + " | right: " + rect_parent.right + " | top: " + rect_parent.top + " | bottom: " + rect_parent.bottom);
                if (playOnlyFirstVideo) {
                    ...
                    for (int i = firstVisiblePosition; i <= lastVisiblePosition; i++) {
                        ...
                        try {
                            AAH_CustomViewHolder1 cvh = (AAH_CustomViewHolder1) holder;
...
//                                    Log.d("trace", i + " foundFirstVideo: " + cvh.getVideoUrl());
                                    ...
                                    if (AAH_Utils1.getString(_act, cvh.getVideoUrl()) != null && new File(AAH_Utils1.getString(_act, cvh.getVideoUrl())).exists()) {
                                        ((AAH_CustomViewHolder1) holder).initVideoView(AAH_Utils1.getString(_act, cvh.getVideoUrl()), _act);
                                    } else {
                                        ((AAH_CustomViewHolder1) holder).initVideoView(cvh.getVideoUrl(), _act);
                                    }
                                    if (downloadVideos) {
                                    ...
                                    Runnable myRunnable = new Runnable() {
                                        @Override
                                        public void run() {
                                            if (!((AAH_CustomViewHolder1) holder).isPaused())
                                                ((AAH_CustomViewHolder1) holder).playVideo();
                                        }
                                    };
...
//                                    Log.d("trace", i + " not foundFirstVideo: ");
                                    ((AAH_CustomViewHolder1) holder).pauseVideo();
                                }
                            }
                        } catch (Exception e) {
                        }
...
//                                Log.d("trace", i + " contains: " + rect_parent.contains(rect_child));
    ...
    public void startDownloadInBackground(String url) {
        if (!AAH_Utils1.isConnected(_act)) return;
        /* Starting Download Service */
        if ((AAH_Utils1.getString(_act, url) == null || !(new File(AAH_Utils1.getString(_act, url)).exists())) && url != null && !url.equalsIgnoreCase("null")) {
    ...
    public void preDownload(List<String> urls) {
        ...
        for (int i = 0; i < urls.size(); i++) {
            if ((AAH_Utils1.getString(_act, urls.get(i)) == null || !(new File(AAH_Utils1.getString(_act, urls.get(i))).exists())) && urls.get(i) != null && !urls.get(i).equalsIgnoreCase("null")) {
                Intent intent = new Intent(Intent.ACTION_SYNC, null, _act, AAH_DownloadService.class);
    ...
    public void stopVideos() {
        for (int i = 0; i < getChildCount(); i++) {
            if (findViewHolderForAdapterPosition(i) instanceof AAH_CustomViewHolder1) {
                final AAH_CustomViewHolder1 cvh = (AAH_CustomViewHolder1) findViewHolderForAdapterPosition(i);
